      ...
      <script src="${contextPath}/static/proscenium/js/jquery.easing.1.3.js" type="text/javascript"></script>
       ...
       <script src="${contextPath}/static/proscenium/js/jquery.gridly.js" type="text/javascript"></script>
        ...
        <link href="./static/proscenium/css/dialog.css" rel="stylesheet" type="text/css"/>
         ...
         <link href="${contextPath}/static/proscenium/css/grid.css" rel="stylesheet" type="text/css" />      
         <script src="${contextPath}/static/proscenium/js/grid.js"></script>
         <link href="${contextPath}/static/proscenium/css/showLoading.css" rel="stylesheet" type="text/css" />      
         <script src="${contextPath}/static/proscenium/js/jquery.showLoading.min.js"></script>
         <script src="${contextPath}/static/proscenium/js/date.js"></script>
          
          
      <script src="${contextPath}/static/proscenium/mmGrid-master/examples/js/plugins.js"></script>
	  <script src="${contextPath}/static/proscenium/mmGrid-master/examples/js/vendor/json2.js"></script>
	  <script src="${contextPath}/static/proscenium/mmGrid-master/src/mmGrid.js"></script>
	  <script src="${contextPath}/static/proscenium/mmGrid-master/src/mmPaginator.js"></script>
	  <!-- 
	  <link rel="stylesheet" href="${contextPath}/static/proscenium/mmGrid-master/examples/css/normalize.css">
      <link rel="stylesheet" href="${contextPath}/static/proscenium/mmGrid-master/examples/css/main.css">
      <link rel="stylesheet" href="${contextPath}/static/proscenium/mmGrid-master/examples/css/960.css">
       -->
      <link href="./static/proscenium/mmGrid-master/src/mmGrid.css" rel="stylesheet" type="text/css"/>
      <link href="./static/proscenium/mmGrid-master/src/mmPaginator.css" rel="stylesheet" type="text/css"/>
      
      <script src="${contextPath}/static/proscenium/js/tableGrid.js"></script>
 ...
 .top2 {
  ...
  .jq_tips_left2{
   ...
   .jq_tips_info2{
	   ...
	   border-radius:4px;padding:5px 8px;max-width:260px;
...
<style>         	
			  ...
			  .jq_tips_left2{
			   ...
			   /*课表提示*/
			   .tip.jq_tips_box2.jq_tips_box_xs2.customRight.TipRight{
			   		left:150px;
			   }
	...
	<body   style="">
	 ...
	 <div  class="min  c"  id="content">
           ...
           <div  class="" style="min-width:1036px;padding:0px;border:0px;">
		        ...
		        <div class="dl-tab-conten">
		        	<ul  >
		        	  ...
		        	  <!--科研 -->	
		        	  <li  class="dl-tab-item" style="display:none;"   ajaxUrl="searchlInformationAjax2.html"  delayLoad="true">
		        	  		<i class="fa fa-spinner fa-spin"   style="font-size:50px;color:blue;margin:20px;"></i>         	 
		        	  </li>
		        	  ...
		        	  <!-- 设备信息 -->
		        	  <li  class="dl-tab-item" style="display:none;"    ajaxUrl="deviceInformationAjax.html"  delayLoad="true">		        	 
		        	  		<i class="fa fa-spinner fa-spin"   style="font-size:50px;color:blue;margin:20px;"></i> 
		        	  </li>
		        	  
		        	  <!--   财务信息 -->
		        	   <li  class="dl-tab-item" style="display:none;"    ajaxUrl="financeInformationAjax.html"  delayLoad="true">		        	 
		        	  		<i class="fa fa-spinner fa-spin"   style="font-size:50px;color:blue;margin:20px;"></i> 
		        	  </li>
		        	  <!--   健康信息 -->
		        	   <li  class="dl-tab-item" style="display:none;"    ajaxUrl="healthInformationAjax.html"  delayLoad="true">		        	 
		        	  		<i class="fa fa-spinner fa-spin"   style="font-size:50px;color:blue;margin:20px;"></i> 
		        	  </li>
		...
		<div id="share">
		  ...
		  <!-- 头部 -->
        <div  class="main2"  id="fixHead2"  style="  display:none;position:fixed;left:0px;padding:10px;z-index:10000;top:100px;" >
           <div  class="article">
	           	<div  class="top2">
...
$(function(){
	...
	// $(".messageTip ").tipsStatic();
	 ...
	 //回到顶部
	     ...
	     // $(document).on("mousemove",".line .lineUnit  .labelValue",function(event){
	    ...
	    //  });
		  ...
		  $(document).on("click", ".viewDetail", function(){ 
			  var  dialogTitle=$(this).closest(".nav-wrapper").find(".nav h3").text();
			  var content='<div  class="table  notOne  dialog"  style="">';
			  var lineTitle=$(this).closest(".table").find(".firstLine .lineUnit");
			  var title=[]
			  lineTitle.each(function(i,d){
				  if(i!=0){
					  title.push($(d).text());
				  }
			  });
			  ...
			  var contentLine=[]
			  var line=$(this).closest(".line").find(".lineUnit");
			  line.each(function(i,d){
				  if(i!=0){
					  contentLine.push($(d).text());
				  }
			  });
			 //  var  lineNumber=1;
			 // var doFirst=false;
			  for(var i=0;i<title.length;){
				 ...
				 if(i==0){//第一行
					 ...
					 if((i+1)<title.length){
						 ...
						 content+=getLine([ [title[i],contentLine[i]],[title[i+1],contentLine[i+1]] ],true);
						 i+=2;
					 }else{//本行只有一个元素
						 content+=getLine([ [title[i],contentLine[i]],['','','none'] ],true);
						 i+=1;
					 }
				 }else{
					 if((i+1)<title.length){
						 content+=getLine([ [title[i],contentLine[i]],[title[i+1],contentLine[i+1]] ]);
						 i+=2;
					 }else{
						 content+=getLine([ [title[i],contentLine[i]],['','','none'] ]);
						 i+=1;
					 }
			  ...
			  //计算dialog的高度和宽度
			  //正常仅仅能容纳4行，超过的的一行为33像素，给height:300添加。
			  
			  
			//  top.showTipThenDo(dialogTitle,content,function(){
				//	top.closeMyDialog();
				//});
			  
			  top.ShowYesNoDialog({title:dialogTitle,content:content,
				    width:650,
				  buttons:[
			  ...
			  //由于dialog 的content类和呗主页的content类覆盖
			  //在dialog显示之前要执行一个函数，或写入一个css
			...
			//  $(".viewDetail").on("myOwnEvent", function(event, showName){
			//    $(this).text(showName + "! What a beautiful name!").show();
			//  });
		//  $(".viewDetail").on("click", {msg: "You just clicked me!"}, function(){
			//	  $(this).trigger("myOwnEvent",["Anja"]);
				  
			//  });
		// 　$(".viewDetail").off("click");
		  /*
		   $("p").on({
		  ...
		  */
		 // $(".viewDetail").on("click", {msg: "You just clicked me!"}, handlerName);
		 // function handlerName(event, showName) 
		//  {
